...
class NewsDAO {
    ...
    suspend fun getTopStoriesByCategory(category: String, context: Context, imageDAO: ImagaDAO): List<NewsItem> = mutex.withLock {
        ...
        for (item in newItems) {
            ...
            val id = savedDao.saveNewsAndReturnId(item)
            if (id != null && item.imageUrl != null) {
                try {
                    val tags = imageDAO.getTags(item.imageUrl)
                    item.imageTags.addAll(tags.map { TagWrapper(it) })
                    savedDao.addTags(tags, id)
                } catch (_: Exception) {}
            }
        }
    ...
    suspend fun loadInitialNews(context: Context, imageDAO: ImagaDAO = RetrofitInstance.defaultImagaDAO) {
        mutex.withLock {
            ...
            for (item in initialNews) {
                val existingId = savedDao.getNewsIdByUuid(item.uuid)
                if (existingId == null) {
                    val newId = savedDao.saveNewsAndReturnId(item)
                    if (newId != null && item.imageUrl != null) {
                        try {
                            val tags = imageDAO.getTags(item.imageUrl)
                            item.imageTags.addAll(tags.map { TagWrapper(it) })
                            savedDao.addTags(tags, newId)
                        } catch (e: Exception) {
                            Log.e("NewsDAO", "Greska za ${item.uuid}: ${e.message}")
                        }
                    }
                }
            }
